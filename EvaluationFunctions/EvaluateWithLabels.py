...
def evaluate_with_labels(actual_drift_points, detected_drift_points, acceptance_levels):
    ...
    mean_time_until_detection = (sum(detection_times) / len(detection_times)) if len(detection_times) > 0 else 0
    ...
    for acceptance_level in acceptance_levels:
        ...
        # true_positives['TP_' + str(acceptance_level)] = TP.item()
        true_positives['TP_' + str(acceptance_level)] = TP
        false_positives['FP_' + str(acceptance_level)] = FP
        false_negatives['FN_' + str(acceptance_level)] = FN
        precisions['Precision_' + str(acceptance_level)] = precision
        recalls['Recall_' + str(acceptance_level)] = recall
        f1scores['F1Score_' + str(acceptance_level)] = f1score
